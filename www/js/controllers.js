angular.module('starter.controllers', []).controller('loginController', function($scope, $ionicPopup, $user, $sync, $location) {
    ...
    //var libraries = [];
    var templates = [];
    secondWave.push(cornerPocket.db.query("templates/groupByTag", {
        include_docs: true
                ...
                if (outOfDateComponents.length > 0) {
                    ...
                    $scope.updateComponents = function() {
                        ...
                        $scope.componentUpdate.update().then(function(res) {
                            ...
                            $scope.view.activeLibrary.selected = null;
    ...
    //use these later
    cornerPocket.db.query("componentSchemas/forLibraryId", {
        include_docs: true
        ...
        for (var prop in byLibrary) {
            ...
            for (var i = 0; i < tags.length; i++) {
                var components = _.filter(byLibrary[prop], function(component) {
                    return component[$user.settings.groupBy] === tags[i]
                });
    ...
    $scope.copyComponent = function() {
        ...
        cornerPocket.doc(copy).then(function(newComponent) {
            ...
            if (newComponent.libraryId === $scope.view.activeLibrary._id) {
                ...
                $timeout(function() {
                    $scope.view.activeLibrary.selected = _.findWhere($scope.components, {
        ...
        modalScope.ok = function() {
            ...
            if (!modalScope.view.existing && !modalScope.itemToUpdate) { //if new
                ...
                cornerPocket.db.post(newItem).then(function(data) {
                    ...
                    //console.log(data);
                    $timeout(function() {
                        var newComponent = _.findWhere($scope.components, {
                            _id: data.id
                        });
                        if (newComponent.libraryId === $scope.view.activeLibrary._id) {
                            //console.log("set selected");
    ...
    $ionicModal.fromTemplateUrl('photos.html', {}).then(function(modal) {
        ...
        modalScope.loadImages = function() {
            ...
            cornerPocket.db.get(modalScope.component._id, {
                attachments: true
            }).then(function(doc) {
            ...
            $ionicPopup.prompt({
                ...
                cornerPocket.db.putAttachment(component._id, res, component._rev, imageData, 'image/jpeg', function(err, result) {
                    if (err) {
                        ...
                        setTimeout(function() {
                            console.log("Saved image: " + component.name + nextPhoto + ".jpg");
                        }, 100);
                    }
                });
            });
