'use strict';
/* Directives */
angular.module('starter.directives', []).directive('focus', function() {
    return {
        restrict: 'A',
    ...
    return {
        restrict: 'A',
        link: function(scope, element, attrs) {
            $ionicGesture.on('tap click', function(e) {}, element);
        }
    }
    ...
    return {
        ...
        template: '<div class="autocomplete {{attrs.class}}" id="{{attrs.id}}">' + '<input type="text" tabindex="101" ng-model="searchParam" placeholder="{{attrs.placeholder}}" class="{{attrs.inputclass}}" id="{{attrs.inputid}}" clear-button/>' + '<div class="arrow" ng-show="(completing && filteredList.length > 0)">▲</div>' + '<ul ng-show="(completing && filteredList.length > 0)" class=\'card list\'>' + '<li suggestion ng-repeat="suggestion in filteredList = (suggestions | filter:searchFilter | orderBy:\'toString()\') track by $index"' + 'index="{{$index}}" val="{{suggestion}}" ng-class="{active: ' + '($index == selectedIndex)}" io-tap="select(suggestion)" ' + 'ng-bind-html="suggestion | highlight:searchParam" class="item">' + '</li>' + '</ul>' + '</div>'
